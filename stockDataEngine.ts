...
export function enrichWithIndicators(data: StockDataPoint[]): StockDataPoint[] {
  ...
  // Map EMAs
  for (let i = 0; i < steps; i++) {
    if (ema12[i] !== undefined) data[i].ema12 = Number(ema12[i].toFixed(2));
    if (ema26[i] !== undefined) data[i].ema26 = Number(ema26[i].toFixed(2));
  }
  ...
  const macdLine: number[] = [];
  for (let i = 0; i < steps; i++) {
    const e12 = ema12[i] !== undefined ? ema12[i] : prices[i];
    const e26 = ema26[i] !== undefined ? ema26[i] : prices[i];
